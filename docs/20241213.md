...
```
1. 谓词下推 是数据库查询优化中的一种技术，它将查询中的谓词（通常是 WHERE 子句、JOIN 条件等）
尽早地应用到数据源或表扫描的过程中（这个过程即下推），以减少需要处理的数据量，从而提高查询效率。
...
优化过程：如果表 employees 是一个大表，数据库优化器会将 salary > 50000 这个谓词推到扫描操作之前。
优化后的执行计划会首先扫描 employees 表，并且只读取满足 salary > 50000 条件的行。
